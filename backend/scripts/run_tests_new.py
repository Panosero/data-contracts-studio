...
class TestRunner:
    ...
    def run_unit_tests(self, verbose: bool = False) -> bool:
        ...
        """
        cmd = ["python", "-m", "pytest", str(self.tests_dir / "unit"), "-m", "unit", "--tb=short"]
    ...
    def _run_command(self, cmd: List[str], test_name: str, fail_on_error: bool = True) -> bool:
        ...
        try:
            result = subprocess.run(cmd, cwd=self.backend_dir, capture_output=False, check=False)
...
def main() -> None:
    ...
    if args.lint_only or args.type == "lint":
        ...
        tests_ok = runner.run_all_tests(verbose=args.verbose, coverage=not args.no_coverage)
