...
class TestAutoGenerationService:
    ...
    def test_sanitize_field_name(
        ...
        assert result == expected_output, (
            f"Expected '{input_field}' to be sanitized to '{expected_output}', "
            f"but got '{result}'"
        )
    
    @pytest.mark.unit
    def test_sanitize_field_name_unicode(
        self, 
        auto_generation_service: AutoGenerationService
    ) -> None:
        ...
        for input_field, expected in test_cases:
            ...
            assert result == expected, (
                f"Expected '{input_field}' to be sanitized to '{expected}', "
                f"but got '{result}'"
            )
    
    @pytest.mark.unit
    def test_sanitize_field_name_length_limits(
        self, 
        auto_generation_service: AutoGenerationService
    ) -> None:
